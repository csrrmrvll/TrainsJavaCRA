...
Unzip TrainsJavaCRA.zip, enter the directory TrainsJavaCRA, open a console and run:

PROMPT>mvn install

in the directory, tests are automatically triggered.
...
arbitrarily to 10 (needed to stop the search). At first I implementd a Dijkstra's Shortest Path algorithm, but sorting roads
on their distances as a preprocessing stage was enough, and maybe even this step is not needed, just a minor optimization that helped me whie debugging.
...
is so small that no appreciable performance penalty is involved have prevented me from attempting a further approach.
